...
Notebooks: `notebooks/analysis/analysis_spatial_complement_C5_C1q_Cfb.ipynb` (in-house Xenium AD, Xenium EAE, Visium aging, plus the Falcão scRNA-seq check) and `notebooks/analysis/analysis_public_datasets_complement.ipynb` (eight public datasets, see [`public_datasets.md`](public_datasets.md)). Each notebook ends with a written interpretation; this page is the condensed version.
...
Human C4A/C4B are not quantifiable in the two MS snRNA-seq datasets (fewer than 25 UMIs in 17–66 k nuclei): the paralogs are near-identical and multi-mapping reads are discarded by Cell Ranger.

## C4b itself

- Rises in oligodendrocytes with age in every aging dataset (pseudobulk log2FC ≈ 2–3), is far higher in aged white than grey matter (77 % of aged white-matter oligodendrocytes C4b⁺ in Kaya et al. 2022), and is up ~2.8 log2 in 5XFAD oligodendrocytes.
...
- Human white matter: a low glial C5 transcript exists (2–7 % of oligodendrocytes, OPCs and astrocytes in Absinta and Jäkel) and goes down, not up, in MS oligodendrocytes (Jäkel log2FC −1.5, p = 0.004).

## C5a receptors

- C5ar1 is myeloid in every dataset: 20–45 % of microglia / macrophages / infiltrating myeloid cells in EAE, 6–38 % of microglia in the public data (highest in aged white matter), 11–15 % at chronic active MS lesion edges. Oligodendrocytes, including C4b⁺ DA oligodendrocytes, are ≤1 % positive in all eleven datasets, and sorted single cells (Falcão) confirm this is not spill-over.
...
- Silent everywhere else: ≤0.2 % of microglia and ≤0.1 % of oligodendrocytes in every AD and aging dataset, ~0.4 % of Visium spots, 6 counts in 66 k human MS nuclei. Cfb therefore marks active inflammatory demyelination rather than the age- or amyloid-associated C4b⁺ state.
...
3. Coarse annotation of the public datasets is marker-based for Park, aging snRNA-seq, Zhou and Kaya; only the oligodendrocyte and microglia labels were validated (Plp1 ≈ 100 %, Hexb 92–100 %). Park OPC / neuron labels are unreliable.
